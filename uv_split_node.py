...
根据一张 UV 网格图从完整贴图中提取对应部件的局部贴图（纯黑背景+padding）。

输入：
  uv_mesh      : UV 网格图（透明背景+白色线框，LoadImage 加载后白线在黑底上）
  uv_texture   : 完整 UV 贴图（RGB）
  padding_px   : 边缘洪泛填充像素数（默认 15）
  close_kernel : 形态学闭运算核大小，弥合线条断裂（默认 3）
  min_area     : 忽略面积小于此值的噪点（像素²，默认 100）
...
class UVPatchExtractNode:
    ...
    def process(self, uv_mesh, uv_texture, padding_px, close_kernel, min_area):
        ...
        black = np.zeros((tex_h, tex_w, 3), dtype=np.uint8)
        black[padded_rgba[:, :, 3] > 127] = padded_rgba[:, :, :3][padded_rgba[:, :, 3] > 127]

        patch_t = torch.from_numpy(black.astype(np.float32) / 255.0).unsqueeze(0)
        mask_t  = torch.from_numpy(solid_mask.astype(np.float32) / 255.0).unsqueeze(0)

        return (patch_t, mask_t)


NODE_CLASS_MAPPINGS      = {"UVPatchExtract": UVPatchExtractNode}
NODE_DISPLAY_NAME_MAPPINGS = {"UVPatchExtract": "UV Patch Extract"}
